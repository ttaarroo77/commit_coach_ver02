...
export const addTaskToProject = (taskTitle: string, projectId: string, groupId: string, taskId?: string): void => {
  const dashboardData = getDashboardData()
...
export const addTaskToDashboard = (
  ...
  if (projectTitle) {
    ...
    // タスクをプロジェクトに追加
    addTaskToProject(taskTitle, targetProject.id, groupId, taskData?.id)
  } else {
...
// サブタスクをタスクに追加
export const addSubtaskToTask = (subtaskTitle: string, taskId: string, projectId: string, groupId: string, subtaskId?: string): void => {
  const dashboardData = getDashboardData()
  const group = dashboardData.find((g) => g.id === groupId)
  if (!group) return
