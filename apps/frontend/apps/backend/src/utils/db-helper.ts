  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116') {
        ...
        throw new ApiError(
          500,
          `Database error in ${table}: ${error.message}`,
          error,
          'DATABASE_ERROR'
        );
      }
...
export async function executeTransaction<T>(callback: () => Promise<T>): Promise<T> {
  try {
    ...
    throw new ApiError(500, 'Transaction failed', error as Error, 'TRANSACTION_FAILED');
  }
}
 ...
 */
export async function getRecord<T>(table: string, id: string, userId?: string): Promise<T> {
 ...
 */
export async function createRecord<T>(table: string, data: Record<string, any>): Promise<T> {
