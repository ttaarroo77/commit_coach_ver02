...
export default function DashboardPage() {
  ...
  const handleAddProject = (groupId: string) => {
    ...
    const uniqueId = `project-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
    addProjectToDashboard(uniqueId, projectTitle, groupId)
  ...
  const handleAddTask = (groupId: string, projectId: string) => {
    ...
    addTaskToProject(taskTitle, projectId, groupId, uniqueId)
  ...
  const handleAddSubtask = (groupId: string, projectId: string, taskId: string) => {
    ...
    const uniqueId = `subtask-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`
    ...
    setTimeout(() => {
      ...
      addSubtaskToTask(subtaskTitle, taskId, projectId, groupId, uniqueId)
    }, 0)
  }
                            ...
                            <Card>
                              ...
                              {group.expanded && (
                                <CardContent className="p-4">
                                  <Droppable droppableId={group.id} type="project" isDropDisabled={false}>
                                    {(provided) => (
                                          ...
                                          <Draggable key={project.id} draggableId={project.id} index={projectIndex}>
                                            {(provided) => (
                                              ...
                                              >
                                                ...
                                                {project.expanded && (
                                                  <div>
                                                    <Droppable droppableId={`${group.id}:${project.id}`} type="task" isDropDisabled={false}>
                                                      {(provided) => (
                                                        <div
