...
| **cursor**<br>*(実装担当)* | - フロントエンド実装<br>- テスト駆動開発<br>- リファクタリング | - コンポーネント実装<br>- テスト実装<br>- バグ修正<br>- パフォーマンス改善 | - 実装完了率<br>- テストカバレッジ<br>- バグ修正率 | o3 |
| **windsurf**<br>*(運用担当)* | - CI/CD構築<br>- デプロイ管理<br>- 運用サポート | - ビルド/デプロイ設定<br>- モニタリング構築<br>- ドキュメント管理<br>- 運用サポート | - デプロイ成功率<br>- システム可用性<br>- レスポンスタイム | cursor |
...
> - cursor は *運用に手を出さない*（実装とテストに専念）  
> - windsurf は *設計に手を出さない*（運用とデプロイに専念）
...
gantt
  ...
  section Implementation (cursor)
    フロントエンド実装         :cursor_impl, b1, after a1, 10d
    テスト実装                :b2, after b1, 4d
  section Operations (windsurf)
    CI/CD構築                :windsurf_ops, c1, after a2, 2d
    デプロイ設定              :c2, after b2, 3d
```
...
| **M2** – フロントエンド実装完了 | 05-21 | 主要機能実装完了 | cursor |
| **M3** – テストカバレッジ達成 | 05-31 | カバレッジ80%以上 | cursor |
| **M4** – 本番環境デプロイ | 06-07 | 本番環境での動作確認完了 | windsurf |
...
| **実装開始** | cursor → 実装計画の提出 & レビュー依頼 |
| **デプロイ** | windsurf → デプロイ計画の提出 & 承認依頼 |
